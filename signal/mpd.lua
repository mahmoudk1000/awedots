...
function M:progress()
	awful.spawn.easy_async_with_shell("mpc | awk 'NR==2{gsub(/[%()]/,\"\",$4); print $4}'", function(stdout)
		awesome.emit_signal("mpd::progress", stdout)
	end)
end

function M:update_cover()
	local timestamp = os.time()
	awful.spawn.with_shell("rm -f " .. res_path .. "cover_*.jpg")

	awful.spawn.easy_async_with_shell(
		'ffmpeg -i $XDG_MUSIC_DIR/"$(mpc current -f %file%)" -map 0:1 -c:v copy -y '
			.. res_path
			.. "cover_"
			.. timestamp
			.. ".jpg",
...
function M:start_idleloop()
	awful.spawn.easy_async_with_shell("pkill -f 'mpc idleloop player'", function()
